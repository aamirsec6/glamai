  ...
  return (
    <>
      <PageHero
        ...
        description="Qimma only uses data that helps you get found, qualify leads, and grow. Here's exactly what we take and how it powers your insights."
      />
          ...
          <div className="mt-12 grid gap-6 md:grid-cols-2">
            {DATA_WE_COLLECT.map((block) => (
              <div key={block.category} className="mkt-card p-8">
                ...
                <p className="mt-4 rounded-lg border border-white/10 bg-white/[0.03] px-3 py-2 text-xs text-zinc-400">
                  <strong className="text-zinc-300">Why:</strong> {block.purpose}
                </p>
              </div>
            ))}
          </div>
      ...
      <section className="mkt-section mkt-section-muted">
        <div className="mkt-container">
          ...
          <div className="mkt-card mt-10 overflow-hidden p-0">
            <table className="mkt-data-table w-full text-sm">
              <thead>
                <tr>
                  <th>Data signal</th>
                  <th>Insight you get</th>
                </tr>
              </thead>
              <tbody>
                {DATA_FOR_INSIGHTS.map((row) => (
                  <tr key={row.signal}>
                    <td className="font-medium text-zinc-200">{row.signal}</td>
                    <td className="text-zinc-400">{row.insight}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <section className="mkt-section">
        <div className="mkt-container grid gap-8 md:grid-cols-2">
          <div className="mkt-card flex gap-4 p-8">
            <Shield className="h-10 w-10 shrink-0 text-cyan-400" />
            <div>
              <h3 className="font-bold text-white">You own your data</h3>
              <p className="mkt-body mt-2 text-sm">
                Export or disconnect anytime. We use data only to power your Qimma account and reports.
              </p>
            </div>
          </div>
          <div className="mkt-card flex gap-4 p-8">
            <Lock className="h-10 w-10 shrink-0 text-cyan-400" />
            <div>
